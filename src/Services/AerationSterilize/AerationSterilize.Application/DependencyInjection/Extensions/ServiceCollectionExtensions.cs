...
public static class ServiceCollectionExtensions
{
    public static void AddConfigurationOptions(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        var dataPlanOptions = configuration.GetSection(nameof(DataPlanOptions)).Get<DataPlanOptions>();
        services.AddSingleton(dataPlanOptions);

        var epplusOptions = configuration.GetSection(nameof(EpplusOptions)).Get<EpplusOptions>();
        services.AddSingleton(epplusOptions);

        var redisOptions = configuration.GetSection(nameof(RedisOptions)).Get<RedisOptions>();
        services.AddSingleton(redisOptions);

        var elasticsearchOptions = configuration.GetSection(nameof(ElasticsearchOptions)).Get<ElasticsearchOptions>();
